...
namespace Music.Assets.Scripts // To change correctly
{
    ...
    {
        private static UniversalAudio _instance; // Variable reference for the class to use instance at the occurrence 
        private static Dictionary<string, float> _audioClipTimerDictionary; // Keys are clips name, Values are the seconds where
        //the clip will start
        ...
        private static readonly float MinimumSecondsToBeSong = 14f;// A song lasts at least 14 seconds according to this line

        private static AudioSource _audioSource1, _audioSource2;

        private IEnumerator FadeTrack(string clipName, bool fromStart) // fromStart = true if the song has to begin from the start
        {
            ...
            if (!fromStart) // If the track has to resume from the point of interruption
            {
                ...
                {
                    _audioSource2.clip = Resources.Load<AudioClip>(clipName);
                    _audioSource2.time = _audioClipTimerDictionary[clipName]; // (The incoming track starts from the value
                                                                              // displayed in _audioClipTimerDictionary)
                    _audioSource2.Play(); 
                    
                    while (timeElapsed < timeOfFading) // ...bring its volume progressively to 0, while the second AudioSource does a Fade in
                    {
                        // Mathf.Lerp does a linear interpolation. It's the simplest way
                        _audioSource2.volume = Mathf.Lerp(startFadeInVolume, finalFadeInVolume, timeElapsed / timeOfFading);
                        _audioSource1.volume = Mathf.Lerp(startFadeOutVolume, finalFadeOutVolume, timeElapsed / timeOfFading);
                ...
                {
                    _audioSource1.clip = Resources.Load<AudioClip>(clipName);
                    _audioSource1.time = _audioClipTimerDictionary[clipName];
                    _audioSource1.Play();
                    while (timeElapsed < timeOfFading)
                    {
                        _audioSource1.volume = Mathf.Lerp(startFadeInVolume, finalFadeInVolume, timeElapsed / timeOfFading);
                        _audioSource2.volume = Mathf.Lerp(startFadeOutVolume, finalFadeOutVolume, timeElapsed / timeOfFading);
                        timeElapsed += Time.deltaTime;
                        yield return null;
                    }
        ...
        private static void InitDictionary()
        {
            if (_audioClipTimerDictionary == null) // If not already initialized
            {
                ...
                foreach (var clipName in Resources.LoadAll(""))
                {
                    if (Resources.Load<AudioClip>(clipName.name).length > MinimumSecondsToBeSong)
                    {
        ...
        private static void PlaySound(string clipName, Transform transform)
        {
            if (clipName.Equals("GameOver")) // The GameOver is treated as a Sound that obliterate all other AudioSources
            {
                StopAllMusic();
            }
        ...
        private static void StopAllMusic() // To stop all the AS
        {
        ...
        private static IEnumerator UpdateTime() // Iterator to update every deltaTime the value of the clips in the _audioClipTimerDictionary
        // if they exist and if an AS is playing the considered clip
        {
            if (_audioSource1.isPlaying && _audioClipTimerDictionary.ContainsKey(_audioSource1.clip.name))
            {
                ...
                if (_audioClipTimerDictionary[_audioSource1.clip.name] >=
                    Resources.Load<AudioClip>(_audioSource1.clip.name).length - 0.01) // 0.01 only for superstition
                {
            ...
            if (_audioSource2.isPlaying && _audioClipTimerDictionary.ContainsKey(_audioSource2.clip.name))
            {
                ...
                if (_audioClipTimerDictionary[_audioSource2.clip.name] >=
                    Resources.Load<AudioClip>(_audioSource2.clip.name).length - 0.01)
                {
